...
export default function History() {
  ...
  const coinOptions = [...new Set(historyData.map((item) => item.coin))].map(
    (coin) => ({
      value: coin,
  ...
  const fromOptions = [
    ...new Set(historyData.map((item) => item.sendingWallet)),
  ].map((wallet) => ({
    value: wallet,
    label: wallet,
  }));
  ...
  const toOptions = [
    ...new Set(historyData.map((item) => item.receivingWallet)),
  ].map((wallet) => ({
    value: wallet,
    label: wallet,
  }));
            ...
            {selectedSection === "Transaction" ? (
              <div>
                <Input
                  ...
                  label="From"
                  options={fromOptions.filter(
                    (option) => option.value !== selectedTo
                  )}
                />
                <Input
                  type="dropdown"
                  ...
                  label="To"
                  options={toOptions.filter(
                    (option) => option.value !== selectedFrom
                  )}
                />
              </div>
            ) : (
      ...
      >
        ...
        >
          ...
          {filteredData.map((row, index) => {
            const senderColor =
              row.sendingWallet === "Trading Wallet" ||
              row.sendingWallet === "Funding Wallet"
                ? "#FFFFFF"
                : "#21DB9A";

            const receiverColor =
              row.receivingWallet === "Trading Wallet" ||
              row.receivingWallet === "Funding Wallet"
                ? "#FFFFFF"
                : "#21DB9A";
              ...
              <TableRow
                ...
                data={
                  selectedSection === "Transaction"
                    ? [
                        <Coin>{row.coin}</Coin>,
                        new Date(parseFloat(row.date)).toLocaleString("en-GB", {
                          dateStyle: "medium",
                          timeStyle: "short",
                          hour12: true,
                        }),
